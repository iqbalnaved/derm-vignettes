...
def load_vignettes(file_path, source_label):
    with open(file_path, 'r') as f:
        data = json.load(f)
    vignettes, labels, sources, case_numbers = [], [], [], []
    valid_diseases = [
        "melanoma",
        "nevus",
    ...
    for item in data:
        ...
        if diag in valid_diseases:
            ...
            case_numbers.append(item.get("case_number", None))
    return vignettes, labels, sources, case_numbers    

# Load datasets
v_img, l_img, s_img, c_img = load_vignettes(
    "/mnt/d/Naved/Data/derm_vignette/Supplementary_Text_1_10132025.json", "With image"
)
v_txt, l_txt, s_txt, c_txt = load_vignettes(
...
def plot_disease(df, diagnosis, filename, annotate_cases=False):
    ...
    for source in ["With image", "Without image"]:
        ...
        # --- Annotate case numbers for "With image" group ---
        if annotate_cases and source == "With image":
            for idx, (i, row) in enumerate(sdata.iterrows(), start=1):
                plt.text(
                    row["TSNE1"] + 1.5,  # horizontal offset
                    row["TSNE2"] + 1.5,  # vertical offset
                    f"Case {row["Case"]}",
                    fontsize=8,
                    color="red",
                    weight="bold",
...
for diag in shape_map.keys():
    plot_disease(df, diag, f"{output_dir}/tsne_{diag.replace(' ', '_').lower()}_{timestamp}.png", annotate_cases=True)
